...
func runA2AAgent(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "usage: bubblefish a2a agent <subcommand>")
	...
	case "show":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: bubblefish a2a agent show <name>")
			os.Exit(1)
		}
		runA2AAgentShow(args[1])
	case "test":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: bubblefish a2a agent test <name>")
			os.Exit(1)
		}
		runA2AAgentTest(args[1])
	case "suspend":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: bubblefish a2a agent suspend <name>")
			os.Exit(1)
		}
		runA2AAgentStatusChange(args[1], registry.StatusSuspended)
	case "retire":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: bubblefish a2a agent retire <name>")
			os.Exit(1)
		}
		runA2AAgentStatusChange(args[1], registry.StatusRetired)
	default:
		fmt.Fprintf(os.Stderr, "bubblefish a2a agent: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}
...
func openA2AStore() *registry.Store {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish a2a: resolve home dir: %v\n", err)
		os.Exit(1)
	}

	dbDir := filepath.Join(home, ".bubblefish", "Nexus", "a2a")
	if err := os.MkdirAll(dbDir, 0700); err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish a2a: create database directory: %v\n", err)
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish a2a: open database: %v\n", err)
		os.Exit(1)
	}
	return store
}
...
func runA2AAgentAdd(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			...
			if name == "" && !strings.HasPrefix(args[i], "-") {
				...
				fmt.Fprintf(os.Stderr, "bubblefish a2a agent add: unknown flag %q\n", args[i])
				os.Exit(1)
			}
		}
	}
	...
	if name == "" {
		fmt.Fprintln(os.Stderr, "usage: bubblefish a2a agent add <name> --transport http --url <url> [--auth bearer]")
		os.Exit(1)
	}
	if tKind == "" {
		tKind = "http"
	...
	if err := tcfg.Validate(); err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish a2a agent add: %v\n", err)
		os.Exit(1)
	}
...
func runA2AAgentList() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish a2a agent list: %v\n", err)
		os.Exit(1)
	}
...
func runA2AAgentShow(name string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish a2a agent show: %v\n", err)
		os.Exit(1)
	}
...
func runA2AAgentTest(name string) {
	...
	agent, err := store.GetByName(ctx, name)
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish a2a agent test: %v\n", err)
		os.Exit(1)
	}
...
func runA2AAgentStatusChange(name, status string) {
	...
	agent, err := store.GetByName(ctx, name)
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish a2a agent %s: %v\n", status, err)
		os.Exit(1)
	}
