...
func runA2AAudit(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "usage: bubblefish a2a audit <subcommand>")
	...
	default:
		fmt.Fprintf(os.Stderr, "bubblefish a2a audit: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}
...
func runA2AAuditTail(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			fmt.Fprintf(os.Stderr, "bubblefish a2a audit tail: unknown flag %q\n", args[i])
			os.Exit(1)
		}
	}
	...
	if since != "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "bubblefish a2a audit tail: invalid duration %q: %v\n", since, err)
			os.Exit(1)
		}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish a2a audit tail: %v\n", err)
		os.Exit(1)
	}
