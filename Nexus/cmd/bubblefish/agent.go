...
// Usage:
//
//	bubblefish agent register --name <name> [--description <desc>]
...
func runAgent(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "usage: bubblefish agent <subcommand>")
	...
	case "show":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: bubblefish agent show <agent_id>")
			os.Exit(1)
		}
		runAgentShow(args[1])
	case "suspend":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: bubblefish agent suspend <agent_id>")
			os.Exit(1)
		}
		runAgentSuspend(args[1])
	case "retire":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: bubblefish agent retire <agent_id>")
			os.Exit(1)
		}
...
func openAgentRegistry() (*agent.Registry, *sql.DB) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish agent: resolve home dir: %v\n", err)
		os.Exit(1)
	}

	dbPath := filepath.Join(home, ".bubblefish", "Nexus", "nexus.db")
	db, err := sql.Open("sqlite", dbPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish agent: open database: %v\n", err)
		os.Exit(1)
	}
	...
	if err != nil {
		db.Close()
		fmt.Fprintf(os.Stderr, "bubblefish agent: init registry: %v\n", err)
		os.Exit(1)
	}
...
func runAgentRegister(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			fmt.Fprintf(os.Stderr, "bubblefish agent register: unknown flag %q\n", args[i])
			os.Exit(1)
		}
	}

	if name == "" {
		fmt.Fprintln(os.Stderr, "usage: bubblefish agent register --name <name> [--description <desc>]")
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish agent register: %v\n", err)
		os.Exit(1)
	}

	fmt.Printf("bubblefish agent register: ok\n")
...
func runAgentList() {
	...
	agents, err := reg.List()
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish agent list: %v\n", err)
		os.Exit(1)
	}
...
func runAgentShow(idOrName string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish agent show: %v\n", err)
		os.Exit(1)
	}
	...
	if a == nil {
		fmt.Fprintf(os.Stderr, "bubblefish agent show: agent %q not found\n", idOrName)
		os.Exit(1)
	}
...
func runAgentSuspend(id string) {
	...
	if err := reg.Suspend(id); err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish agent suspend: %v\n", err)
		os.Exit(1)
	}

	fmt.Printf("bubblefish agent suspend: ok — agent %s suspended\n", id)
}
...
func runAgentRetire(id string) {
	...
	if err := reg.Retire(id); err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish agent retire: %v\n", err)
		os.Exit(1)
	}
