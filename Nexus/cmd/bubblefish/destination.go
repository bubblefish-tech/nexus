...
// Usage:
//
//	bubblefish destination rebuild [--name NAME] [--no-backup]
//
// Reference: v0.1.3 Build Plan Section 6.6.
func runDestination(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: bubblefish destination <subcommand>")
	...
	default:
		fmt.Fprintf(os.Stderr, "bubblefish destination: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}

// runDestinationRebuild implements `bubblefish destination rebuild`.
func runDestinationRebuild(args []string) {
	fs := flag.NewFlagSet("bubblefish destination rebuild", flag.ExitOnError)
	...
	if *walDir == "" || *dbPath == "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "bubblefish destination rebuild: %v\n", err)
			os.Exit(1)
		}
		cfg, err := config.Load(configDir, logger)
		if err != nil {
			fmt.Fprintf(os.Stderr, "bubblefish destination rebuild: config load: %v\n", err)
			os.Exit(1)
		}
		...
		if *dbPath == "" {
			// Default SQLite path is ~/.bubblefish/Nexus/nexus.db
			home, _ := os.UserHomeDir()
			*dbPath = filepath.Join(home, ".bubblefish", "Nexus", "nexus.db")
		}
	}

	fmt.Fprintf(os.Stderr, "bubblefish destination rebuild: WAL=%s DB=%s\n", *walDir, *dbPath)
	...
	fmt.Fprintf(os.Stderr, "bubblefish destination rebuild: ok — %d entries written (%s)\n",
