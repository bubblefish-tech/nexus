...
// Usage:
//
//	bubblefish import <path> [--format auto|claude-zip|chatgpt-zip|claude-code-dir|cursor-dir|jsonl]
//	                         [--source-name <custom>]
//	                         [--dry-run]
//	                         [--url <daemon URL>]
//	                         [--api-key <key>]
func runImport(args []string) {
	fs := flag.NewFlagSet("bubblefish import", flag.ExitOnError)
	...
	if fs.NArg() < 1 {
		fmt.Fprintln(os.Stderr, "bubblefish import: path argument is required")
		fmt.Fprintln(os.Stderr, "usage: bubblefish import <path> [--format auto] [--dry-run] [--source-name custom]")
	...
	if !*dryRun && *apiKey == "" {
		fmt.Fprintln(os.Stderr, "bubblefish import: --api-key or NEXUS_API_KEY is required (or use --dry-run)")
		os.Exit(1)
	}
	...
	fmt.Fprintf(os.Stderr, "bubblefish import: importing %s (format=%s)\n", path, *format)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish import: %v\n", err)
		os.Exit(1)
	}

	if *dryRun {
		fmt.Fprintf(os.Stderr, "bubblefish import: DRY RUN — %d memories found, 0 written\n", result.Total)
	} else {
		fmt.Fprintf(os.Stderr, "bubblefish import: %d written, %d skipped, %d errored (%.1fs)\n",
