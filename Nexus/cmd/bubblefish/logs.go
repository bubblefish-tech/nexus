...
// runLogs executes the `bubblefish logs` command.
//
...
func runLogs(args []string) {
	...
	if err := fs.Parse(args); err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish logs: %v\n", err)
		os.Exit(1)
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			fmt.Printf("bubblefish logs: no log file at %s (daemon has not run yet)\n", logPath)
			return
		}
		fmt.Fprintf(os.Stderr, "bubblefish logs: open %s: %v\n", logPath, err)
		os.Exit(1)
	}
	...
	if *since != "" {
		...
		if sinceTime.IsZero() {
			fmt.Fprintf(os.Stderr, "bubblefish logs: invalid --since value %q (use RFC3339 or duration like 1h, 30m)\n", *since)
			os.Exit(1)
		}
	}
	...
	if err := scanner.Err(); err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish logs: read: %v\n", err)
		os.Exit(1)
	}
