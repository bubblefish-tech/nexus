...
func runMCPStdio(args []string) {
	...
	homeFlag := fs.String("home", "", "override config directory (defaults to $BUBBLEFISH_HOME or ~/.bubblefish)")
	...
	if err != nil {
		fatalStdio("load config from %s: %v (try 'bubblefish install')", configDir, err)
	}
	...
	if len(cfg.ResolvedMCPKey) == 0 {
		fatalStdio("mcp api_key is empty in daemon.toml. Run 'bubblefish install' or add an api_key under [daemon.mcp]")
	}
	...
	if err := preflight(client, targetURL, authHeader); err != nil {
		fatalStdio("daemon unreachable at %s: %v (is the daemon running? try: bubblefish start)", targetURL, err)
	}
...
func fatalStdio(format string, args ...interface{}) {
	fmt.Fprintf(os.Stderr, "bubblefish mcp stdio: "+format+"\n", args...)
	os.Exit(1)
}
