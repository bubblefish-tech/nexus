...
// runTunnel executes the `bubblefish tunnel` command family.
//
// Subcommands:
//
...
func runTunnel(args []string) {
	...
	case "status":
		runTunnelStatus()
	default:
		fmt.Fprintf(os.Stderr, "bubblefish tunnel: unknown subcommand %q\n", args[0])
		printTunnelUsage()
		os.Exit(1)
	}
}
...
func printTunnelUsage() {
	fmt.Fprintln(os.Stderr, "usage: bubblefish tunnel <subcommand>")
...
func runTunnelSetup() {
	fmt.Println("bubblefish tunnel setup: interactive tunnel configurator")
	fmt.Println()
	...
	default:
		fmt.Fprintf(os.Stderr, "bubblefish tunnel setup: unknown provider %q\n", provider)
		os.Exit(1)
	}
	...
	fmt.Println("  bubblefish stop && bubblefish start")
}
...
func runTunnelDoctor() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish tunnel doctor: %v\n", err)
		os.Exit(1)
	}

	cfg, err := config.Load(configDir, logger)
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish tunnel doctor: config: %v\n", err)
		os.Exit(1)
	}

	if len(cfg.Tunnels) == 0 {
		fmt.Println("bubblefish tunnel doctor: no tunnels configured")
		fmt.Println("  Run 'bubblefish tunnel setup' to add a tunnel.")
		return
	}
	...
	fmt.Println()
	if hasErrors {
		fmt.Println("bubblefish tunnel doctor: issues found — see above")
		os.Exit(1)
	}
	fmt.Println("bubblefish tunnel doctor: ok")
}
...
func runTunnelStatus() {
	...
	configDir, err := config.ConfigDir()
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish tunnel status: %v\n", err)
		os.Exit(1)
	}

	cfg, err := config.Load(configDir, logger)
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish tunnel status: config: %v\n", err)
		os.Exit(1)
	}

	if len(cfg.Tunnels) == 0 {
		fmt.Println("bubblefish tunnel status: no tunnels configured")
		return
	}
