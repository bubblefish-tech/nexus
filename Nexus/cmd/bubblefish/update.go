...
// runUpdate executes the `bubblefish update` command.
//
// Flow:
...
func runUpdate(args []string) {
	...
	if err := fs.Parse(args); err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish update: %v\n", err)
		os.Exit(1)
	}

	client := &http.Client{Timeout: 30 * time.Second}

	fmt.Println("bubblefish update: checking for new release...")
	...
	if !updater.CompareVersions(current, latest) {
		fmt.Println("bubblefish update: already up to date")
		return
	}

	fmt.Printf("bubblefish update: new version available: %s → %s\n", current, latest)

	if *checkOnly {
		fmt.Println("  (run 'bubblefish update' without --check to install)")
		return
	}
	...
	if port := resolveDaemonPort(configDir); port > 0 && checkDaemonAlive(port) {
		...
		fmt.Println("           Run: bubblefish stop")
		if !*yes {
			fmt.Println()
			...
			if answer != "y" && answer != "Y" {
				fmt.Println("bubblefish update: aborted")
				return
			}
		}
	}
	...
	// Download binary.
	fmt.Println("bubblefish update: downloading binary...")
	...
	// Download checksum.
	fmt.Println("bubblefish update: downloading checksum...")
	...
	// Verify checksum.
	fmt.Println("bubblefish update: verifying checksum...")
	...
	// Atomic replace.
	fmt.Printf("bubblefish update: installing %s → %s...\n", exe, latest)
	...
	fmt.Printf("bubblefish update: installed — previous binary saved as %s\n", backupPath)
	fmt.Printf("bubblefish update: ok — upgraded %s → %s\n", current, latest)
	fmt.Println()
	fmt.Println("  Next steps:")
	fmt.Println("  1. Run 'bubblefish start' to start the new version")
	fmt.Println("  2. Run 'bubblefish doctor' to verify the new version is healthy")
