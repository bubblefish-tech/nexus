...
// runVerify executes `bubblefish verify`.
//
// Modes:
//
//	bubblefish verify <proof-file.json>                     — verify a local file
//	bubblefish verify --proof <memory_id> [--url URL]       — fetch proof from daemon
//	bubblefish verify ... --output <path>                   — write HTML (*.html) or JSON
//
// Exit codes:
//
//	0 = valid
...
func runVerify(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		case "--proof":
			i++
			if i >= len(args) {
				fmt.Fprintln(os.Stderr, "bubblefish verify: --proof requires a memory_id argument")
				os.Exit(1)
			}
		...
		case "--output":
			i++
			if i >= len(args) {
				fmt.Fprintln(os.Stderr, "bubblefish verify: --output requires a path argument")
				os.Exit(1)
			}
			outputPath = args[i]
		case "--url":
			i++
			if i >= len(args) {
				fmt.Fprintln(os.Stderr, "bubblefish verify: --url requires a URL argument")
				os.Exit(1)
			}
			daemonURL = args[i]
		case "--token":
			i++
			if i >= len(args) {
				fmt.Fprintln(os.Stderr, "bubblefish verify: --token requires a value")
				os.Exit(1)
			}
			adminToken = args[i]
		default:
	...
	if proofMemoryID == "" && len(positional) == 0 {
		...
		fmt.Fprintln(os.Stderr, "       bubblefish verify --proof <memory_id> [--url URL] [--token TOKEN] [--output proof.html]")
		os.Exit(1)
	}
	...
	if proofMemoryID != "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "bubblefish verify: fetch proof: %v\n", err)
			os.Exit(1)
		}
		bundle = *b
	} else {
	...
	if outputPath != "" {
		if err := writeVerifyOutput(&bundle, outputPath); err != nil {
			fmt.Fprintf(os.Stderr, "bubblefish verify: write output: %v\n", err)
			os.Exit(1)
		}
		fmt.Printf("bubblefish verify: written to %s\n", outputPath)
