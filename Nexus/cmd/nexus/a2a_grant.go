...
func runA2AGrant(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "usage: nexus a2a grant <subcommand>")
	...
	case "revoke":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: nexus a2a grant revoke <grant_id>")
			os.Exit(1)
		}
		runA2AGrantRevoke(args[1])
	default:
		fmt.Fprintf(os.Stderr, "nexus a2a grant: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}
...
func openA2AGrantStore() (*governance.GrantStore, *sql.DB) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a grant: resolve home dir: %v\n", err)
		os.Exit(1)
	}

	dbDir := filepath.Join(home, ".nexus", "Nexus", "a2a")
	if err := os.MkdirAll(dbDir, 0700); err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a grant: create database directory: %v\n", err)
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a grant: open database: %v\n", err)
		os.Exit(1)
	}
	...
	for _, pragma := range []string{
		...
		if _, err := db.Exec(pragma); err != nil {
			db.Close()
			fmt.Fprintf(os.Stderr, "nexus a2a grant: %s: %v\n", pragma, err)
			os.Exit(1)
		}
	}
	...
	if err := governance.MigrateGrants(db); err != nil {
		db.Close()
		fmt.Fprintf(os.Stderr, "nexus a2a grant: migrate: %v\n", err)
		os.Exit(1)
	}
...
func runA2AGrantAdd(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			fmt.Fprintf(os.Stderr, "nexus a2a grant add: unknown flag %q\n", args[i])
			os.Exit(1)
		}
	}

	if source == "" || target == "" || capability == "" {
		fmt.Fprintln(os.Stderr, "usage: nexus a2a grant add --source <src> --target <tgt> --capability <glob> [--expires <duration>]")
		os.Exit(1)
	}
	...
	if expires != "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "nexus a2a grant add: invalid duration %q: %v\n", expires, err)
			os.Exit(1)
		}
		exp := now.Add(dur)
		grant.ExpiresAt = &exp
	...
	if err := store.CreateGrant(grant); err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a grant add: %v\n", err)
		os.Exit(1)
	}
...
func runA2AGrantList(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		case strings.HasPrefix(args[i], "--target="):
			target = strings.TrimPrefix(args[i], "--target=")
		default:
			fmt.Fprintf(os.Stderr, "nexus a2a grant list: unknown flag %q\n", args[i])
			os.Exit(1)
		}
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a grant list: %v\n", err)
		os.Exit(1)
	}
...
func runA2AGrantRevoke(grantID string) {
	...
	if err := store.RevokeGrant(grantID, time.Now()); err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a grant revoke: %v\n", err)
		os.Exit(1)
	}
