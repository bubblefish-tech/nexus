...
func runA2ATask(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "usage: nexus a2a task <subcommand>")
	...
	case "get":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: nexus a2a task get <task_id>")
			os.Exit(1)
		}
		runA2ATaskGet(args[1])
	case "cancel":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: nexus a2a task cancel <task_id>")
			os.Exit(1)
		}
	...
	default:
		fmt.Fprintf(os.Stderr, "nexus a2a task: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}
...
func openA2ATaskStore() *store.SQLiteTaskStore {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a task: resolve home dir: %v\n", err)
		os.Exit(1)
	}

	dbDir := filepath.Join(home, ".nexus", "Nexus", "a2a")
	if err := os.MkdirAll(dbDir, 0700); err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a task: create database directory: %v\n", err)
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a task: open database: %v\n", err)
		os.Exit(1)
	}
	return ts
}
...
func runA2ATaskGet(taskID string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a task get: %v\n", err)
		os.Exit(1)
	}
...
func runA2ATaskCancel(taskID string) {
	...
	if err := ts.UpdateTaskStatus(ctx, taskID, status); err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a task cancel: %v\n", err)
		os.Exit(1)
	}
...
func runA2ATaskList(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		default:
			fmt.Fprintf(os.Stderr, "nexus a2a task list: unknown flag %q\n", args[i])
			os.Exit(1)
		}
	}
	...
	if since != "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "nexus a2a task list: invalid duration %q: %v\n", since, err)
			os.Exit(1)
		}
	...
	if state != "" {
		...
		if !ok {
			fmt.Fprintf(os.Stderr, "nexus a2a task list: unknown state %q\n", state)
			os.Exit(1)
		}
		filter.State = ts
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus a2a task list: %v\n", err)
		os.Exit(1)
	}
