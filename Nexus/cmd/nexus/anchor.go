...
// Usage:
//
//	nexus anchor setup --gist   configure GitHub Gist auto-publish
//
// Reference: v0.1.3 Build Plan Phase 4 Subtask 4.8.
func runAnchor(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "usage: nexus anchor <subcommand>")
	...
	default:
		fmt.Fprintf(os.Stderr, "nexus anchor: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}
...
func runAnchorSetup(args []string) {
	...
	if !useGist {
		fmt.Fprintln(os.Stderr, "usage: nexus anchor setup --gist")
	...
	if _, err := fmt.Scanln(&token); err != nil {
		fmt.Fprintf(os.Stderr, "nexus anchor setup: read token: %v\n", err)
		os.Exit(1)
	}

	if token == "" {
		fmt.Fprintln(os.Stderr, "nexus anchor setup: token cannot be empty")
		os.Exit(1)
	}

	configDir, err := config.ConfigDir()
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus anchor setup: %v\n", err)
		os.Exit(1)
	}
	...
	if err := sd.WriteSecret("gist-token", []byte(token)); err != nil {
		fmt.Fprintf(os.Stderr, "nexus anchor setup: write token: %v\n", err)
		os.Exit(1)
	}

	fmt.Println("nexus anchor setup: ok — GitHub Gist token stored in secrets/gist-token")
