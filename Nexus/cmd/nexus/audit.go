...
// runAudit dispatches `nexus audit <subcommand>`.
//
// Reference: Tech Spec Addendum Section A5.
func runAudit(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: nexus audit <subcommand>")
	...
	default:
		fmt.Fprintf(os.Stderr, "nexus audit: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}
...
func runAuditQuery(args []string) {
	...
	if err := fs.Parse(args); err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit query: parse flags: %v\n", err)
		os.Exit(1)
	}

	reader, err := buildReaderFromConfig()
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit query: %v\n", err)
		os.Exit(1)
	}
	...
	if *after != "" {
		...
		if perr != nil {
			fmt.Fprintf(os.Stderr, "nexus audit query: invalid --after: %v\n", perr)
			os.Exit(1)
		}
		filter.After = t
	}
	if *before != "" {
		...
		if perr != nil {
			fmt.Fprintf(os.Stderr, "nexus audit query: invalid --before: %v\n", perr)
			os.Exit(1)
		}
		filter.Before = t
	}
	...
	if err := enc.Encode(result); err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit query: encode: %v\n", err)
		os.Exit(1)
	}
}
...
func runAuditStats(args []string) {
	...
	if err := fs.Parse(args); err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit stats: parse flags: %v\n", err)
		os.Exit(1)
	}

	reader, err := buildReaderFromConfig()
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit stats: %v\n", err)
		os.Exit(1)
	}
	...
	if err := enc.Encode(stats); err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit stats: encode: %v\n", err)
		os.Exit(1)
	}
}
...
func runAuditExport(args []string) {
	...
	if err := fs.Parse(args); err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit export: parse flags: %v\n", err)
		os.Exit(1)
	}

	if *format != "json" && *format != "csv" {
		fmt.Fprintf(os.Stderr, "nexus audit export: unsupported format %q (use json or csv)\n", *format)
		os.Exit(1)
	}

	reader, err := buildReaderFromConfig()
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit export: %v\n", err)
		os.Exit(1)
	}
	...
	if *after != "" {
		t, perr := time.Parse(time.RFC3339, *after)
		if perr != nil {
			fmt.Fprintf(os.Stderr, "nexus audit export: invalid --after: %v\n", perr)
			os.Exit(1)
		}
		filter.After = t
	}
	if *before != "" {
		t, perr := time.Parse(time.RFC3339, *before)
		if perr != nil {
			fmt.Fprintf(os.Stderr, "nexus audit export: invalid --before: %v\n", perr)
			os.Exit(1)
		}
		filter.Before = t
	}
	...
	if *output != "" {
		...
		fmt.Fprintf(os.Stderr, "nexus audit export: %d records written to %s\n", len(result.Records), *output)
		return
	}
...
func runAuditTail(args []string) {
	...
	if err := fs.Parse(args); err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit tail: parse flags: %v\n", err)
		os.Exit(1)
	}

	reader, err := buildReaderFromConfig()
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit tail: %v\n", err)
		os.Exit(1)
	}
	...
	fmt.Fprintln(os.Stderr, "nexus audit tail: watching for new entries (ctrl+c to stop)")
	...
	for {
		...
		result, err := reader.Query(filter)
		if err != nil {
			fmt.Fprintf(os.Stderr, "nexus audit tail: poll error: %v\n", err)
			continue
		}
...
func runAuditRecover(args []string) {
	...
	reader, err := buildReaderFromConfig()
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit recover: %v\n", err)
		os.Exit(1)
	}
	...
	result, err := reader.Query(filter)
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus audit recover: read audit log: %v\n", err)
		os.Exit(1)
	}
	...
	for _, rec := range result.Records {
		...
		if merr != nil {
			fmt.Fprintf(os.Stderr, "nexus audit recover: marshal record %s: %v\n", rec.RecordID, merr)
			os.Exit(1)
		}
	...
	if report.ChainStatus == "intact" || report.ChainStatus == "empty" {
		fmt.Println("\nnexus audit recover: chain is healthy, no action needed")
		return
	}
	...
	if !scanner.Scan() {
		fmt.Fprintln(os.Stderr, "nexus audit recover: no input, aborting")
		os.Exit(1)
	}
	...
	case "a":
		fmt.Printf("nexus audit recover: truncation acknowledged. "+
		...
		if cerr != nil {
			fmt.Fprintf(os.Stderr, "nexus audit recover: resolve config dir: %v\n", cerr)
			os.Exit(1)
		}
		...
		if serr := cs.SaveChainState(dataDir); serr != nil {
			fmt.Fprintf(os.Stderr, "nexus audit recover: save chain state: %v\n", serr)
			os.Exit(1)
		}
		fmt.Println("nexus audit recover: chain state reset. Restart the daemon to rebuild.")
	case "b":
		fmt.Println("nexus audit recover: aborted, no changes made")
	default:
		fmt.Fprintf(os.Stderr, "nexus audit recover: unrecognized choice %q, aborting\n", choice)
		os.Exit(1)
	}
}
