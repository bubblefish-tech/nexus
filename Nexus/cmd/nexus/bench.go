...
// runBench executes `nexus bench`.
//
// Usage:
//
//	nexus bench --mode throughput [--url URL] [--n N] [--concurrency C] [--output file.json]
//	nexus bench --mode latency   [--url URL] [--n N] [--query Q] [--output file.json]
//	nexus bench --mode eval      [--url URL] [--golden file.json] [--output file.json]
//
// Reference: Tech Spec Section 13.4, Phase R-25.
func runBench(args []string) {
	fs := flag.NewFlagSet("nexus bench", flag.ExitOnError)
	...
	if *mode == "" {
		fmt.Fprintln(os.Stderr, "nexus bench: --mode is required")
		fmt.Fprintln(os.Stderr, "usage: nexus bench --mode <throughput|latency|eval> [flags]")
	...
	if *apiKey == "" && *adminKey == "" {
		fmt.Fprintln(os.Stderr, "nexus bench: --api-key or NEXUS_API_KEY is required")
		os.Exit(1)
	}
