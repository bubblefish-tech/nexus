...
// runChaos executes `nexus chaos`.
//
// Usage:
//
//	nexus chaos --url URL --api-key KEY [flags]
//
// Reference: v0.1.3 Build Plan Section 6.1.
func runChaos(args []string) {
	fs := flag.NewFlagSet("nexus chaos", flag.ExitOnError)
	...
	if *apiKey == "" {
		fmt.Fprintln(os.Stderr, "nexus chaos: --api-key or NEXUS_API_KEY is required")
		os.Exit(1)
	}
	if *dbPath == "" {
		fmt.Fprintln(os.Stderr, "nexus chaos: --db is required (path to memories.db for direct DB verification)")
		os.Exit(1)
	}
	if *adminKey == "" {
		fmt.Fprintln(os.Stderr, "nexus chaos: --admin-key or NEXUS_ADMIN_KEY is required for /admin/memories verification")
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus chaos: %v\n", err)
		os.Exit(1)
	}
	...
	if *report != "" {
		if err := os.WriteFile(*report, out, 0600); err != nil {
			fmt.Fprintf(os.Stderr, "nexus chaos: write report: %v\n", err)
			os.Exit(1)
		}
		fmt.Fprintf(os.Stderr, "nexus chaos: report written to %s\n", *report)
	} else {
