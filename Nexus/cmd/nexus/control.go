...
// runGrant dispatches `nexus grant <subcommand>`.
func runGrant(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: nexus grant <subcommand>")
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus grant: %v\n", err)
		os.Exit(1)
	}
	switch args[0] {
	case "create":
		if err := doGrantCreate(cl, args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "nexus grant create: %v\n", err)
			os.Exit(1)
		}
	case "list":
		if err := doGrantList(cl, args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "nexus grant list: %v\n", err)
			os.Exit(1)
		}
	case "revoke":
		if err := doGrantRevoke(cl, args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "nexus grant revoke: %v\n", err)
			os.Exit(1)
		}
	default:
		fmt.Fprintf(os.Stderr, "nexus grant: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}
...
func doGrantCreate(cl *controlClient, args []string) error {
	...
	if err := parseFlags(args,
		...
		return fmt.Errorf("%v\nusage: nexus grant create --agent <id> --capability <cap> [--scope <json>] [--expires <duration>] [--granted-by <id>] [--json]", err)
	}
	if agent == "" || capability == "" {
...
func doGrantList(cl *controlClient, args []string) error {
	...
	if err := parseFlags(args,
		...
		return fmt.Errorf("%v\nusage: nexus grant list [--agent <id>] [--json]", err)
	}
...
func doGrantRevoke(cl *controlClient, args []string) error {
	...
	if err := parseFlags(args,
		...
		return fmt.Errorf("%v\nusage: nexus grant revoke --id <grant_id> [--reason <text>] [--json]", err)
	}
	if id == "" {
...
// runApproval dispatches `nexus approval <subcommand>`.
func runApproval(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: nexus approval <subcommand>")
		fmt.Fprintln(os.Stderr, "subcommands:")
	...
	cl, err := loadControlClient(os.Stdout, os.Stderr)
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus approval: %v\n", err)
		os.Exit(1)
	}
	switch args[0] {
	case "list":
		if err := doApprovalList(cl, args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "nexus approval list: %v\n", err)
			os.Exit(1)
		}
	case "decide":
		if err := doApprovalDecide(cl, args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "nexus approval decide: %v\n", err)
			os.Exit(1)
		}
	default:
		fmt.Fprintf(os.Stderr, "nexus approval: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}
...
func doApprovalList(cl *controlClient, args []string) error {
	...
	if err := parseFlags(args,
		...
		return fmt.Errorf("%v\nusage: nexus approval list [--status pending|approved|denied] [--json]", err)
	}
...
func doApprovalDecide(cl *controlClient, args []string) error {
	...
	if err := parseFlags(args,
		...
		return fmt.Errorf("%v\nusage: nexus approval decide --id <id> --decision approve|deny [--reason <text>] [--json]", err)
	}
	if id == "" || decision == "" {
...
// runTask dispatches `nexus task <subcommand>`.
func runTask(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: nexus task <subcommand>")
		fmt.Fprintln(os.Stderr, "subcommands:")
	...
	cl, err := loadControlClient(os.Stdout, os.Stderr)
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus task: %v\n", err)
		os.Exit(1)
	}
	switch args[0] {
	case "list":
		if err := doTaskList(cl, args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "nexus task list: %v\n", err)
			os.Exit(1)
		}
	case "inspect":
		if err := doTaskInspect(cl, args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "nexus task inspect: %v\n", err)
			os.Exit(1)
		}
	default:
		fmt.Fprintf(os.Stderr, "nexus task: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}
...
func doTaskList(cl *controlClient, args []string) error {
	...
	if err := parseFlags(args,
		...
		return fmt.Errorf("%v\nusage: nexus task list [--agent <id>] [--state <state>] [--json]", err)
	}
	var params []string
	if agentID != "" {
...
func doTaskInspect(cl *controlClient, args []string) error {
	...
	if err := parseFlags(args,
		...
		return fmt.Errorf("%v\nusage: nexus task inspect --id <task_id> [--json]", err)
	}
	if id == "" {
		return fmt.Errorf("--id is required")
	}
...
// runAction dispatches `nexus action <subcommand>`.
func runAction(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: nexus action <subcommand>")
		fmt.Fprintln(os.Stderr, "subcommands:")
	...
	cl, err := loadControlClient(os.Stdout, os.Stderr)
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus action: %v\n", err)
		os.Exit(1)
	}
	switch args[0] {
	case "log":
		if err := doActionLog(cl, args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "nexus action log: %v\n", err)
			os.Exit(1)
		}
	default:
		fmt.Fprintf(os.Stderr, "nexus action: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}
...
func doActionLog(cl *controlClient, args []string) error {
	...
	if err := parseFlags(args,
		...
		return fmt.Errorf("%v\nusage: nexus action log [--agent <id>] [--capability <cap>] [--since <duration>] [--json]", err)
	}
	var params []string
	if agentID != "" {
		params = append(params, "agent_id="+url.QueryEscape(agentID))
