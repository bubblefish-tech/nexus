...
// runDev executes the `nexus dev` command.
//
// It starts the same daemon as `nexus start` with dev-friendly defaults:
...
func runDev() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus dev: %v\n", err)
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus dev: config load: %v\n", err)
		os.Exit(1)
	}
	...
	if cfg.Daemon.Web.Port > 0 {
		...
		go func() {
			if err := dashboard.Start(); err != nil {
				logger.Warn("nexus dev: web dashboard failed — continuing without dashboard",
					"component", "main",
					"error", err,
				)
			}
	...
	select {
	case sig := <-sigCh:
		logger.Info("nexus dev: received signal, shutting down",
			"component", "main",
			"signal", sig.String(),
		)
	case err := <-daemonErr:
		if err != nil {
			logger.Error("nexus dev: daemon exited with error",
				"component", "main",
				"error", err,
			)
		}
	...
	if err := d.Stop(); err != nil {
		logger.Error("nexus dev: shutdown error",
			"component", "main",
			"error", err,
		)
		os.Exit(1)
	...
	logger.Info("nexus dev: shutdown complete",
		"component", "main",
	)
}
