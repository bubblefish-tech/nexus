...
// runIngest executes `nexus ingest <subcommand>`.
//
// Subcommands:
//
...
func runIngest(args []string) {
	...
	case "pause":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: nexus ingest pause <watcher>")
			os.Exit(1)
		}
		ingestControl(url, adminKey, args[1], "pause")
	case "resume":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: nexus ingest resume <watcher>")
			os.Exit(1)
		}
		ingestControl(url, adminKey, args[1], "resume")
	case "reset":
		if len(args) < 2 {
			fmt.Fprintln(os.Stderr, "usage: nexus ingest reset <watcher> [<path>]")
			os.Exit(1)
		}
		path := ""
		if len(args) > 2 {
	...
	default:
		fmt.Fprintf(os.Stderr, "nexus ingest: unknown subcommand %q\n", args[0])
		printIngestUsage()
		os.Exit(1)
	}
}
...
func printIngestUsage() {
	fmt.Fprintln(os.Stderr, "usage: nexus ingest <subcommand>")
...
func ingestStatus(baseURL, adminKey string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus ingest status: %v\n", err)
		os.Exit(1)
	}
	...
	if err := json.Unmarshal(body, &resp); err != nil {
		fmt.Fprintf(os.Stderr, "nexus ingest status: parse response: %v\n", err)
		os.Exit(1)
	}
...
func ingestControl(baseURL, adminKey, watcher, action string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus ingest %s: %v\n", action, err)
		os.Exit(1)
	}
...
func ingestReset(baseURL, adminKey, watcher, path string) {
	...
	body, err := ingestAPIPost(url, adminKey)
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus ingest reset: %v\n", err)
		os.Exit(1)
	}
	fmt.Println(string(body))
}
