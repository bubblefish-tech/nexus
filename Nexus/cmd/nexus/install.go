...
// runInstall executes the `nexus install` command.
//
// It creates the config directory tree (~/.nexus/Nexus/) with daemon.toml,
...
func runInstall(args []string) {
	...
	if err := fs.Parse(args); err != nil {
		fmt.Fprintf(os.Stderr, "nexus install: %v\n", err)
		os.Exit(1)
	}
...
// --home flag > BUBBLEFISH_HOME env var > default (~/.nexus/Nexus).
...
func doInstall(opts installOptions) error {
	...
	_, _ = fmt.Fprintf(w, "nexus install: ok -- v%s\n", version.Version)
	...
	if opts.mode == "simple" {
		...
		_, _ = fmt.Fprintln(w, "     nexus start")
		_, _ = fmt.Fprintln(w)
		...
		_, _ = fmt.Fprintln(w, "  2. Build config:   nexus build")
		_, _ = fmt.Fprintln(w, "  3. Start daemon:   nexus start")
		_, _ = fmt.Fprintln(w, "  4. Health check:   nexus doctor")
	}
	return nil
}
...
func generateKey(prefix string) string {
	...
	if _, err := rand.Read(b); err != nil {
		fmt.Fprintf(os.Stderr, "nexus install: generate key: %v\n", err)
		os.Exit(1)
	}
