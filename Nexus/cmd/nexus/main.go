...
// Command nexus is the entry point for BubbleFish Nexus.
package main
...
func main() {
	if len(os.Args) < 2 {
		...
		fmt.Fprintln(os.Stderr, "usage: nexus <command>")
		...
		fmt.Fprintln(os.Stderr, "  stop         gracefully stop a running nexus daemon")
	fmt.Fprintln(os.Stderr, "  status       show daemon health and resolved paths")
		...
		fmt.Fprintln(os.Stderr, "  drift        continuous drift detection between WAL and destinations")
		...
		fmt.Fprintln(os.Stderr, "  grant        manage control-plane capability grants")
		...
		fmt.Fprintln(os.Stderr, "  update       check for and install new versions")
		...
		fmt.Fprintln(os.Stderr, "  config       manage Nexus configuration (set-password, encrypt, decrypt)")
	...
	case "mcp":
		if len(os.Args) < 3 {
			fmt.Fprintln(os.Stderr, "usage: nexus mcp <subcommand>")
		...
		default:
			fmt.Fprintf(os.Stderr, "nexus mcp: unknown subcommand %q\n", os.Args[2])
			os.Exit(1)
		}
	case "setup":
		runSetup()
	...
	case "config":
		runConfig(os.Args[2:])
	case "set-password", "set_password":
	...
	case "version", "--version":
		fmt.Printf("nexus nexus v%s (pre-1.0, API subject to change)\n", version.Version)
	default:
		fmt.Fprintf(os.Stderr, "nexus: unknown command %q\n", os.Args[1])
		fmt.Fprintln(os.Stderr, "usage: nexus <install|start|stop|dev|build|lint|doctor|status|audit|backup|bench|demo|sign-config|mcp|version>")
		os.Exit(1)
	}
}

// runMCPTest executes the `nexus mcp test` command.
//
...
func runMCPTest() {
	const testKey = "nexus-mcp-self-test-key"

	// Find a free loopback port.
	ln, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus mcp test: find free port: %v\n", err)
		os.Exit(1)
	}
	...
	if err := srv.Start(); err != nil {
		fmt.Fprintf(os.Stderr, "nexus mcp test: start failed: %v\n", err)
		os.Exit(1)
	}
	defer func() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus mcp test: build request: %v\n", err)
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus mcp test: call nexus_status: %v\n", err)
		os.Exit(1)
	}
	...
	if err := json.Unmarshal(b, &result); err != nil {
		fmt.Fprintf(os.Stderr, "nexus mcp test: parse response: %v\n", err)
		os.Exit(1)
	}

	if result["error"] != nil {
		fmt.Fprintf(os.Stderr, "nexus mcp test: nexus_status returned error: %v\n", result["error"])
		os.Exit(1)
	}
	...
	if len(content) == 0 {
		fmt.Fprintln(os.Stderr, "nexus mcp test: empty content in response")
		os.Exit(1)
	}
	...
	if err := json.Unmarshal([]byte(text), &statusResult); err != nil {
		fmt.Fprintf(os.Stderr, "nexus mcp test: parse status: %v\n", err)
		os.Exit(1)
	}
	if statusResult["status"] != "ok" {
		fmt.Fprintf(os.Stderr, "nexus mcp test: unexpected status=%v\n", statusResult["status"])
		os.Exit(1)
	}

	fmt.Printf("nexus mcp test: ok — nexus_status returned status=%q version=%v\n",
		statusResult["status"], statusResult["version"])
}

// runSignConfig executes the `nexus sign-config` command.
...
//
// Usage: nexus sign-config --keyref env:NEXUS_SIGNING_KEY
//
...
func runSignConfig() {
	...
	if keyRef == "" {
		fmt.Fprintln(os.Stderr, "nexus sign-config: --keyref is required")
		fmt.Fprintln(os.Stderr, "usage: nexus sign-config --keyref env:NEXUS_SIGNING_KEY")
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus sign-config: resolve key: %v\n", err)
		os.Exit(1)
	}
	if resolved == "" {
		fmt.Fprintln(os.Stderr, "nexus sign-config: key resolved to empty value")
		os.Exit(1)
	}

	configDir, err := config.ConfigDir()
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus sign-config: %v\n", err)
		os.Exit(1)
	}
	...
	fmt.Println("nexus sign-config: ok — all compiled config files signed")
}

// runLint executes the `nexus lint` command.
...
func runLint() {
	...
	configDir, err := config.ConfigDir()
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus lint: %v\n", err)
		os.Exit(1)
	}

	cfg, err := config.Load(configDir, logger)
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus lint: config load failed: %v\n", err)
		os.Exit(1)
	}
	...
	if len(result.Findings) == 0 {
		fmt.Println("nexus lint: ok — no issues found")
		return
	}
	...
	if result.HasErrors() {
		fmt.Fprintf(os.Stderr, "\nnexus lint: %d issue(s) found, including errors\n", len(result.Findings))
		os.Exit(1)
	}
	fmt.Printf("\nnexus lint: %d warning(s), no errors\n", len(result.Findings))
}

// runBuild executes the `nexus build` command.
...
func runBuild() {
	...
	configDir, err := config.ConfigDir()
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus build: %v\n", err)
		os.Exit(1)
	}
	...
	fmt.Printf("nexus build: ok — compiled/policies.json written\n")
}
