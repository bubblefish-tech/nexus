...
// runQuarantine dispatches the `nexus quarantine` subcommands.
func runQuarantine(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: nexus quarantine <subcommand>")
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus quarantine: %v\n", err)
		os.Exit(1)
	}
	switch args[0] {
	case "list":
		if err := doQuarantineList(cl, args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "nexus quarantine list: %v\n", err)
			os.Exit(1)
		}
	case "approve":
		if err := doQuarantineDecide(cl, args[1:], "approve"); err != nil {
			fmt.Fprintf(os.Stderr, "nexus quarantine approve: %v\n", err)
			os.Exit(1)
		}
	case "reject":
		if err := doQuarantineDecide(cl, args[1:], "reject"); err != nil {
			fmt.Fprintf(os.Stderr, "nexus quarantine reject: %v\n", err)
			os.Exit(1)
		}
	default:
