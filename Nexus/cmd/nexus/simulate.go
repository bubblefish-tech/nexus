...
// runSimulate executes `nexus simulate`.
//
// Reference: v0.1.3 Build Plan Section 6.2.
func runSimulate(args []string) {
	fs := flag.NewFlagSet("nexus simulate", flag.ExitOnError)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus simulate: %v\n", err)
		os.Exit(1)
	}
	...
	if *report != "" {
		if err := os.WriteFile(*report, out, 0600); err != nil {
			fmt.Fprintf(os.Stderr, "nexus simulate: write report: %v\n", err)
			os.Exit(1)
		}
		fmt.Fprintf(os.Stderr, "nexus simulate: report written to %s\n", *report)
	} else {
