...
// runStatus executes the `nexus status` command.
func runStatus(args []string) {
	...
	if err := fs.Parse(args); err != nil {
		fmt.Fprintf(os.Stderr, "nexus status: %v\n", err)
		os.Exit(1)
	}
	...
	if *paths {
		if err := doStatusPaths(opts); err != nil {
			fmt.Fprintf(os.Stderr, "nexus status --paths: %v\n", err)
			os.Exit(1)
		}
		return
	}
...
func doStatusDefault(opts statusOptions) error {
	...
	if err != nil {
		return fmt.Errorf("no config found at %s. Run 'nexus install' first", opts.configDir)
	}

	bind := fmt.Sprintf("%s:%d", cfg.Daemon.Bind, cfg.Daemon.Port)

	fmt.Fprintf(opts.stdout, "nexus status (v%s)\n\n", version.Version)
	...
	if err != nil {
		...
		fmt.Fprintf(opts.stdout, "  For path information, run: nexus status --paths\n")
		return nil
	}
	_ = resp.Body.Close()
	...
	if err != nil {
		...
		fmt.Fprintf(opts.stdout, "\n  For path information, run: nexus status --paths\n")
		return nil
	}
...
func doStatusPaths(opts statusOptions) error {
	...
	fmt.Fprintf(opts.stdout, "nexus paths (v%s)\n\n", version.Version)
