...
// runStop executes the `nexus stop` command.
...
func doStop(opts stopOptions) int {
	...
	if err != nil {
		fmt.Fprintf(opts.stderr, "nexus stop: no config found at %s. Run 'nexus install' first.\n", opts.configDir)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(opts.stdout, "nexus stop: no daemon running at %s\n", bind)
		return 0
	}
	...
	if err != nil {
		fmt.Fprintf(opts.stderr, "nexus stop: %v\n", err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(opts.stderr, "nexus stop: failed to send shutdown request: %v\n", err)
		return 1
	}
	_ = resp.Body.Close()

	if resp.StatusCode == http.StatusUnauthorized {
		fmt.Fprintf(opts.stderr, "nexus stop: admin token rejected. Check that the daemon was started from the same config directory.\n")
		return 1
	}

	if resp.StatusCode != http.StatusAccepted {
		fmt.Fprintf(opts.stderr, "nexus stop: unexpected response %d from /api/shutdown\n", resp.StatusCode)
		return 1
	}

	fmt.Fprintf(opts.stdout, "nexus stop: shutdown requested, waiting for daemon to exit...\n")
	...
	for time.Now().Before(deadline) {
		...
		if err != nil {
			...
			fmt.Fprintf(opts.stdout, "nexus stop: daemon stopped cleanly\n")
			return 0
		}
	}

	fmt.Fprintf(opts.stderr, "nexus stop: daemon did not exit within %d seconds. You may need to kill it manually with: taskkill /F /IM nexus.exe (Windows) or kill -9 <pid> (POSIX).\n", opts.timeout)
	return 1
}
