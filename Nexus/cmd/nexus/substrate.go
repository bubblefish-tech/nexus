...
// runSubstrate handles the `nexus substrate` command family.
func runSubstrate(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: nexus substrate <subcommand>")
	...
	default:
		fmt.Fprintf(os.Stderr, "nexus substrate: unknown subcommand %q\n", args[0])
		os.Exit(1)
	}
}
...
func runSubstrateProveDeletion(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "usage: nexus substrate prove-deletion <memory_id>")
		os.Exit(1)
	}
	memoryID := args[0]
...
func runSubstrateShred(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "usage: nexus substrate shred <memory_id>")
		os.Exit(1)
	}
	memoryID := args[0]
	...
	fmt.Println(string(body))
	fmt.Printf("\nDeletion proof available via: nexus substrate prove-deletion %s\n", memoryID)
}
