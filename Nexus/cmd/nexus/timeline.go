...
// policy decisions.
//
// Usage: nexus timeline <memory_id>
//
...
func runTimeline(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "usage: nexus timeline <memory_id>")
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus timeline: %v\n", err)
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus timeline: read audit log: %v\n", err)
		os.Exit(1)
	}
	...
	if len(matching) == 0 {
		fmt.Printf("nexus timeline: no audit records found for %s\n", memoryID)
		return
	}
