...
# Search order: BUBBLEFISH_HOME env var, NEXUS_HOME env var, then common paths
$NexusHome = $null
$searchPaths = @()

if ($env:BUBBLEFISH_HOME) { $searchPaths += $env:BUBBLEFISH_HOME }
if ($env:NEXUS_HOME)      { $searchPaths += $env:NEXUS_HOME }
$searchPaths += Join-Path $env:USERPROFILE ".nexus\Nexus"
$searchPaths += Join-Path $env:USERPROFILE ".bubblefish\Nexus"
$searchPaths += "D:\Test\BubbleFish\v010-dogfood\home"

foreach ($candidate in $searchPaths) {
    $testFile = Join-Path $candidate "daemon.toml"
...
if (-not $NexusHome) {
    Write-Host "      daemon.toml not found. Searched:" -ForegroundColor Red
    foreach ($p in $searchPaths) {
        Write-Host "        $p" -ForegroundColor Red
    }
    Write-Host "      Set BUBBLEFISH_HOME or NEXUS_HOME and try again." -ForegroundColor Red
    exit 1
}
