...
// MigrateEncryptionColumns adds the CU.0.4/CU.0.6 encrypted-column sets to all
// relevant tables in an existing database. It is safe to call on a database
// created from the current SchemaSQL (which already includes these columns) —
// duplicate-column errors from SQLite are silently ignored. New installations
// never need this call; it exists for databases created before these columns
// were merged.
func MigrateEncryptionColumns(db *sql.DB) error {
	stmts := []string{
		// a2a_agents (CU.0.6)
		`ALTER TABLE a2a_agents ADD COLUMN agent_card_json_encrypted BLOB`,
		`ALTER TABLE a2a_agents ADD COLUMN transport_toml_encrypted  BLOB`,
		`ALTER TABLE a2a_agents ADD COLUMN last_error_encrypted      BLOB`,
		`ALTER TABLE a2a_agents ADD COLUMN encryption_version        INTEGER NOT NULL DEFAULT 0`,
		// grants (CU.0.4)
