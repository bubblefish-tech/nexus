...
func TestDriftDetectsAnomaly(t *testing.T) {
	...
	if d.AnomaliesTotal() == 0 {
		t.Error("expected anomalies > 0 for missing entry p2")
	}
}

func TestDriftAllPresent(t *testing.T) {
	mw := &mockWAL{entries: []wal.Entry{
		{PayloadID: "p1"},
		{PayloadID: "p2"},
	}}
	...
	if d.AnomaliesTotal() != 0 {
		t.Errorf("expected 0 anomalies, got %d", d.AnomaliesTotal())
	}
}

func TestDriftStopDrains(t *testing.T) {
	mw := &mockWAL{}
	md := &mockDest{existing: map[string]bool{}}
