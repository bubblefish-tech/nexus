...
// Usage:
//
//	bubblefish demo [--url URL] [--api-key KEY] [--admin-key KEY] [--source S] [--destination D] [--keep]
//
...
func runDemo(args []string) {
	fs := flag.NewFlagSet("bubblefish demo", flag.ExitOnError)
	...
	if *apiKey == "" {
		fmt.Fprintln(os.Stderr, "bubblefish demo: --api-key or NEXUS_API_KEY is required")
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "bubblefish demo: %v\n", err)
		os.Exit(1)
	}
	...
	if result.Pass {
		fmt.Fprintln(os.Stderr, "bubblefish demo: PASS — 50 present, 0 duplicates")
	} else {
		fmt.Fprintf(os.Stderr, "bubblefish demo: FAIL — recovered=%d, duplicates=%d, missing=%d\n",
